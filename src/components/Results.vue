  ...
  <div class="results-container">
    ...
    <div class="blue-green-test-result-text w-full mt-0 bg-white">
      <p class="result-text" v-if="testGroup === 'control'">
        <i>你</i>对蓝-绿色彩对感知的分界点位于hue={{ Math.round(displayThreshold) }}，在蓝绿色轴中位于{{ displayPropText }}处
      </p>
      <p class="result-text" v-else-if="testGroup === 'social'">
        <i>你</i>对蓝-绿色彩对感知的分界点位于hue={{ Math.round(displayThreshold) }}，
...
export default {
  ...
  methods: {
    ...
    calculateDisplayValues() {
      ...
      this.userThresholdBias = this.displayThreshold < middlePoint ? '绿' : '蓝'
      this.invertedBias = this.userThresholdBias === '蓝' ? '绿' : '蓝'
    },
    createPlot() {
