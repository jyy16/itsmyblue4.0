<template>
  <div class="blue-purple-test-wrapper">
    <div :style="containerStyle" class="blue-purple-test-container">
      <div v-if="!showResults" class="blue-purple-test-content">
        <transition name="fade-up" mode="out-in">
          <h1 v-if="showInitialMessage" key="initial" class="blue-purple-test-title">
            <span class="background-white">测试<i>你的</i>色彩分类</span>
          </h1>
          <h1 v-else key="main" class="blue-purple-test-title">
            <span class="background-white"><i>我的</i>蓝色是<i>你的</i>蓝色吗？</span>
          </h1>
        </transition>
      </div>
      <div v-else class="blue-purple-test-content blue-purple-test-result-screen">
        <Results
      ...
      <div v-if="rounds < MAX_ROUNDS" class="blue-purple-test-button-container three-buttons">
        <button @click="selectColor(leftButton)" class="blue-purple-test-button grow-button">
          这是{{ leftButton === 'blue' ? '蓝色' : '紫色' }}
        </button>
        <button @click="handleResetClick" class="blue-purple-test-button mid-reset-button grow-button">
          {{ resetButtonText }}
        </button>
        <button @click="selectColor(rightButton)" class="blue-purple-test-button grow-button">
          这是{{ rightButton === 'blue' ? '蓝色' : '紫色' }}
        </button>
      </div>
      <div v-else class="blue-purple-test-button-container two-buttons">
        <button
          @click="showAbout = true"
          class="blue-purple-test-button final-reset-button grow-button"
        >
          关于
        </button>
        <button @click="emitNextTest" class="blue-purple-test-button final-reset-button grow-button">
          下一个测试
        </button>
      </div>
    </div>
    ...
    <div
      v-if="rounds == MAX_ROUNDS && (allSame == 'blue' || allSame == 'purple')"
      class="about-popup"
    >
      <div class="about-content">
        <h2>哎呀！</h2>
        <p>
          你将所有色彩都标记为{{ allSame === 'blue' ? '蓝色' : '紫色' }}。我们无法根据你的
    ...
    <div v-if="showAbout" class="about-popup">
      <div class="about-content">
        ...
        <p>
          ...
          Language can affect how we memorize and name colors. This is a color naming test designed to measure your personal blue-purple boundary.
        </p>
        <h2>Test validity</h2>
        <p>
        ...
        <p>
          ...
          (hue, saturation, lightness) color space. Hue 240 is blue, and hue 270 is purple. The test
          focuses on blue-purple hues between 240 and 300. On the web, HSL coordinates are translated
        ...
        <p>
          The test assumes that your responses between blue and purple are represented by a sigmoid
        ...
        <h2>Results</h2>
        <p>
          In early experiments with blue-green boundaries, people's responses clustered around 175.
          For blue-purple boundaries, the nominal boundary is around 270, which is the hue value for
          pure purple <span class="color-chip-purple mr-1"></span>, while pure blue
          <span class="color-chip-blue mr-1"></span> is at 240.
        </p>
...
<script setup>
import { ref } from 'vue'
import { MAX_ROUNDS, BIN_POSITION, BIN_COUNT, X_CDF, Y_CDF } from '@/config_for_BP' // Adjust the import path as needed
...
<script>
import { MAX_ROUNDS, VERSION, BIN_POSITION, BIN_COUNT, X_CDF, Y_CDF } from '@/config_for_BP'
import confetti from 'https://cdn.skypack.dev/canvas-confetti'
import Results from './Results_for_BPTest.vue'
...
export default {
  ...
  data() {
    return {
      currentHue: Math.random() > 0.5 ? 240 : 270,
      ...
      showAbout: false,
      showDemo: false,
      purpleButtonRight: Math.random() > 0.5,
  ...
  computed: {
    rightButton() {
      return this.purpleButtonRight ? 'purple' : 'blue'
    },
    leftButton() {
      return this.purpleButtonRight ? 'blue' : 'purple'
    },
    currentColor() {
  ...
  methods: {
    emitNextTest() {
      const testData = {
        ...
        testPosition: this.$props.testPosition,
    ...
    selectColor(color) {
      ...
      if (this.rounds === 0) {
        if (color === 'blue' && this.currentHue > 255) { // 修改边界值
          this.firstColorMislabeled = true
        } else if (color === 'purple' && this.currentHue < 255) { // 修改边界值
          this.firstColorMislabeled = true
        }
      ...
      const { a, b, polarity, newProbe } = fitSigmoid(
        ...
        this.responses.map(r => r.response === 'purple'),
        this.polarity,
        0.4,
        255, // 修改中心值
        240, // 修改最小值
      ...
      // 强制边界约束
      this.currentHue = Math.max(240, Math.min(this.currentHue, 270)); // 修改边界值

      this.rounds++;
      if (this.rounds === MAX_ROUNDS) {
        if (
          this.responses.every((r) => r.response === 'blue') ||
          this.responses.every((r) => r.response === 'purple')
        ) {
          this.allSame = this.responses[0].response
          return
        }
        this.finalHue = 255 - b // 修改中心值
    ...
    reset() {
      let currentHue = Math.random() > 0.5 ? 240 : 270 // 修改随机初始值范围
      if (this.firstColorMislabeled) {
        currentHue = this.currentHue == 270 ? 240 : 270 // 修改边界值
      }
...
<style src="./BluePurpleTest.css" scoped />
<style scoped>
.color-chip-purple {
