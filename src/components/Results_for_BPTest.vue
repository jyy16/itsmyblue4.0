  ...
  <div class="results-container">
    <div class="svg-container">
      ...
      <div class="absolute top-0 left-0 p-1">
        <div class="blue-purple-test-result-color">
          <p class="result-text bg-white bg-opacity-70 p-1 rounded"><i>你的</i> 蓝色</p>
        </div>
      </div>
      <div class="absolute top-0 right-0 p-1">
        <div class="blue-purple-test-result-color">
          <p class="result-text bg-white bg-opacity-70 p-1 rounded"><i>你的</i> 紫色</p>
        </div>
      </div>
    </div>
    <div class="blue-purple-test-result-text w-full mt-0 bg-white">
      <p class="result-text" v-if="testGroup === 'control'">
        <i>你</i>对蓝-紫色彩对感知的分界点位于hue={{ Math.round(displayThreshold) }}，在蓝紫色轴中位于{{ displayPropText }}处
      </p>
      <p class="result-text" v-else-if="testGroup === 'social'">
        <i>你</i>对蓝-紫色彩对感知的分界点位于hue={{ Math.round(displayThreshold) }}，
...
export default {
  ...
  computed: {
    ...
    blueInclusive() {
      ...
      const blueInclusive = index !== -1 ? this.yCdf[index] : 1
      return blueInclusive
    }
  },
  mounted() {
  ...
  methods: {
    ...
    calculateDisplayValues() {
      ...
      // 根据显示阈值确定偏向色彩，而不是用户实际阈值
      this.userThresholdBias = this.displayThreshold < middlePoint ? '蓝' : '紫'
      this.invertedBias = this.userThresholdBias === '蓝' ? '紫' : '蓝'
    },
    createPlot() {
