  ...
  <div class="blue-green-test-wrapper">
    <div :style="containerStyle" class="blue-green-test-container">
      ...
      <div v-if="rounds < MAX_ROUNDS" class="blue-green-test-button-container three-buttons">
        <button @click="selectColor(leftButton)" class="blue-green-test-button grow-button">
          这是{{ leftButton === 'blue' ? '蓝色' : '绿色' }}
        </button>
        <button @click="handleResetClick" class="blue-green-test-button mid-reset-button grow-button">
          {{ resetButtonText  }}
        </button>
        <button @click="selectColor(rightButton)" class="blue-green-test-button grow-button">
          这是{{ rightButton === 'blue' ? '蓝色' : '绿色' }}
        </button>
      </div>
      <div v-else class="blue-green-test-button-container two-buttons">
        ...
        <button @click="emitNextTest" class="blue-green-test-button final-reset-button grow-button">
          下一个测试
        </button>
      </div>
    </div>
    ...
    >
      <div class="about-content">
        <h2>哎呀！</h2>
        <p>
          你将所有色彩都标记为{{ allSame === 'blue' ? '蓝色' : '绿色' }}。我们无法根据你的
...
<script setup>
import { ref } from 'vue'
import { MAX_ROUNDS, BIN_POSITION, BIN_COUNT, X_CDF, Y_CDF } from '@/config.js' // Adjust the import path as needed
...
<script>
import { MAX_ROUNDS, VERSION, BIN_POSITION, BIN_COUNT, X_CDF, Y_CDF } from '@/config.js'
...
export default {
  ...
  methods: {
    ...
    reset() {
      let currentHue = Math.random() > 0.5 ? 120 : 240
      if (this.firstColorMislabeled) {
        currentHue = this.currentHue == 240 ? 120 : 240
      }
